...
# folder will be created next to this script when you run it
VIOLATIONS_DIR = Path("speed_violations")
VIOLATION_CROP_PADDING_PX = 10

# HLS output for web streaming
# Path to Laravel public/stream folder for detection output
HLS_OUTPUT_DIR = Path(r"D:\tapo\tapo\public\stream")
...
ffmpeg_path = r"D:\tapo\tapo\ffmpeg-2026-01-22-git-4561fc5e48-full_build\ffmpeg-2026-01-22-git-4561fc5e48-full_build\bin\ffmpeg.exe"
ffmpeg_command = [
    ffmpeg_path,
...
# map of ('car', id) -> last center y (for vertical speed)
id_to_last_cy = {}
# map of ('car', id) -> 'left' or 'right' (movement direction)
id_to_direction = {}
...
# map of ('car', id) -> last exit speed in km/h
id_to_exit_speed_kmh = {}
# map of ('car', id) -> frame_idx until we show it
id_to_exit_speed_until = {}
...
while True:
    ...
    # Convert CUDA tensor to CPU before creating DataFrame
    ...
    for _, row in px.iterrows():
        ...
        y2 = int(row[3])
        cls_idx = int(row[5])
        cls_name = class_list[cls_idx] if 0 <= cls_idx < len(
            class_list) else ''
    ...
    cv2.line(frame, (LEFT_ENTRY_LINE_X, 0),
             (LEFT_ENTRY_LINE_X, h - 1), entry_color, 4)
    cv2.line(frame, (RIGHT_ENTRY_LINE_X, 0),
             (RIGHT_ENTRY_LINE_X, h - 1), entry_color, 4)
    cv2.line(frame, (LEFT_EXIT_LINE_X, 0),
             (LEFT_EXIT_LINE_X, h - 1), exit_color, 4)
    cv2.line(frame, (RIGHT_EXIT_LINE_X, 0),
             (RIGHT_EXIT_LINE_X, h - 1), exit_color, 4)
    ...
    left_exit_label_x = max(
        0, min(w - 2 * exit_label_half_width, left_exit_label_x))
    right_exit_label_x = max(
        0, min(w - 2 * exit_label_half_width, right_exit_label_x))
        ...
        for bbox in boxes:
            ...
            if prev_cx is not None:
                ...
                if direction == 'right':
                    ...
                    crossed_entry = (prev_cx < LEFT_ENTRY_LINE_X - LINE_TOLERANCE) and (
                        cx >= LEFT_ENTRY_LINE_X + LINE_TOLERANCE)
                    crossed_exit = (prev_cx < RIGHT_EXIT_LINE_X - LINE_TOLERANCE) and (
                        cx >= RIGHT_EXIT_LINE_X + LINE_TOLERANCE)
                else:  # direction == 'left'
                    # Moving left: check right entry and left exit
                    crossed_entry = (prev_cx > RIGHT_ENTRY_LINE_X + LINE_TOLERANCE) and (
                        cx <= RIGHT_ENTRY_LINE_X - LINE_TOLERANCE)
                    crossed_exit = (
                        prev_cx > LEFT_EXIT_LINE_X + LINE_TOLERANCE) and (cx <= LEFT_EXIT_LINE_X - LINE_TOLERANCE)
                ...
                if crossed_exit:
                    ...
                    if v_x is not None:
                        ...
                        id_to_exit_speed_until[key] = frame_idx + \
                            SHOW_EXIT_SPEED_FOR_FRAMES
...
# Final stats
print(
    f"Entries: cars={entries['car']}, buses={entries['bus']}, trucks={entries['truck']}")
print(
    f"Exits:   cars={exits['car']}, buses={exits['bus']}, trucks={exits['truck']}")
print(
    f'Final on-bridge count: {len(on_bridge_ids)} | Load: {current_load_tons:.1f} tons')
