...
# Tapo RTSP stream (change IP/credentials if needed)
...
class ANPR:
    ...
    def infer_video(self, source: str = 0, output_path: str = None, display: bool = True):
        """Performs real-time ANPR on a video stream (file, webcam, or RTSP URL)."""
        cap = cv2.VideoCapture(source)
        if not cap.isOpened():
            raise ValueError(f"Cannot open video source: {source}")
        # Reduce buffer for RTSP to lower latency
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="ANPR - Number Plate Recognition")
    parser.add_argument("--source", "-s", default=TAPO_RTSP, help="Video source: RTSP URL, file path, or 0 for webcam")
    parser.add_argument("--output", "-o", default=None, help="Output video path (optional for RTSP)")
